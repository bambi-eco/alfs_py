...
def project_images_for_flight(flight_key: int, split: str, images_folder: str, labels_folder: str, dem_file: str, poses_file: str, correction_matrix_file: str,
    ...
    try:
        ...
        created_files = []  # Track created files for cleanup if needed
        ...
        for shot, shot_name, shot_rotation_eulers in zip(shots, shot_names, shots_rotation_eulers):
            ...
            for random_z_rotation in random_z_rotations:	
                ...
                created_files.extend([save_name, labels_save_name])
                ...
                for class_id, poly_coords in frame_labels:
                    ...
                    if w_poses[0] is None:
                        logging.error(f"w_poses is None for flight {flight_key}, frame {frame_idx}. Cleaning up and skipping remaining files.")
                        failed_projections[frame_idx] += 1
        ...
        # Clean up created files on any error
        for file in created_files:
            if os.path.exists(file):
                try:
                    os.remove(file)
                    ...
                    logging.error(f"Failed to remove file {file}: {cleanup_error}")
        release_all(ctx)
        raise e
...
if __name__ == "__main__":
    ...
    failed_projections_summary = {}
    # project images for each split
    for split in SPLITS:
        logging.info(f"starting orthografic projection with split {split}")
        failed_projections = project_images_for_split(split, DATASET_DIR, OUTPUT_DIR, ORTHO_WIDTH, ORTHO_HEIGHT, RENDER_WIDTH, RENDER_HEIGHT, CAMERA_DISTANCE, INITIAL_SKIP, ADD_BACKGROUND, FOVY, ASPECT_RATIO, SAVE_LABELED_IMAGES, INPUT_WIDTH, INPUT_HEIGHT, project_orthogonal, ADDITIONAL_ROTATIONS, ROTATION_LIMIT, rng)
        failed_projections_summary[split] = failed_projections
        logging.info(f"done with split {split}")

    # Print summary of failed projections
    logging.info("\nSummary of failed projections:")
    for split, flights in failed_projections_summary.items():
        if flights:
            logging.info(f"\nSplit: {split}")
            for flight_key, frames in flights.items():
                logging.info(f"Flight {flight_key}:")
                for frame_idx, count in frames.items():
                    logging.info(f"  Frame {frame_idx}: {count} failed projections")
