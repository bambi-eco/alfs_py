...
from alfspy.core.util.basic import compare_color
...
def split_components(arr: NDArray) -> tuple[NDArray, ...]:
    ...
    :return: A tuple of slices of the given numpy array.
    """
...
def replace_color(img: NDArray, color_from: Color, color_to: Color, inplace: bool = False) -> NDArray:
    """
    Recolor all pixels of one color to the given color.
    :param img: The image to recolor.
    :param color_from: The current color of the pixels to recolor.
    :param color_to: The color that should be assigned to the pixels.
    :param inplace: Whether recoloring should be done inplace.
...
def integral(images: Sequence[NDArray], dtype=None) -> Optional[NDArray]:
    ...
    :param dtype: The type to use for the resulting array.
    :return: None if no images are passed or if the images have different resolutions; otherwise the blended image.
    """
    img_count = len(images)
    if img_count == 0:
