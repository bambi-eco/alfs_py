...
__all__ = ['shade_object', 'shot_clip_coords', 'shade_shot']
...
def shade_shot(fragments: Fragments, shot_clip: torch.Tensor, indices: torch.Tensor,
    ...
    :return: A tuple of ``(colours, keep)`` where ``colours`` is ``(M, 4)`` RGBA and ``keep``
        is an ``(N,)`` boolean selecting the surviving fragments (the shader's ``discard``).
    """
    count = len(fragments)
    device = fragments.bary.device
    dtype = fragments.bary.dtype
    ...
    uv = clip[:, :3] / safe_w.unsqueeze(-1) / 2.0 + 0.5

    keep = (uv[:, 0] >= 0.0) & (uv[:, 0] <= 1.0) & (uv[:, 1] >= 0.0) & (uv[:, 1] <= 1.0)
