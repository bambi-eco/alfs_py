from enum import Enum

from pyrr import Vector3


class PixelOrigin(Enum):
    """
    Enum representing a 2D coordinate origin.
    The first two bits represent the vertical while the last two represent horizontal positioning:
    - 00xx: top
    - 01xx: vertical center
    - 10xx: bottom
    - xx00: left
    - xx01: horizontal center
    - xx10: right
    """
    TopLeft =      0b0000
    TopCenter =    0b0001
    TopRight =     0b0010

    CenterLeft =   0b0100
    Center =       0b0101
    CenterRight =  0b0110

    BottomLeft =   0b1000
    BottomCenter = 0b1001
    BottomRight =  0b1010


def pixel_to_3d_coord(x: float, y: float, width: int, height: int, camera_position: Vector3, fovy: float,
                      aspect_ratio: float, orthogonal: bool = False, distorted: bool = False, dist_coeffs = None) -> Vector3:
    pass